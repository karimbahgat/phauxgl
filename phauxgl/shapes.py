...
from .util import LatLngToXYZ, Radians
from .mesh import NewTriangleMesh
from .triangle import NewTriangleForPoints
from .matrix import Scale, Translate
...
def NewLatLngSphere(latStep, lngStep):
    triangles = []
    for lat0 in range(-90, 90, latStep):
        lat1 = lat0 + latStep
        v0 = float(lat0+90) / 180
        v1 = float(lat1+90) / 180
        for lng0 in range(-180, 180, lngStep):
            lng1 = lng0 + lngStep
            u0 = float(lng0+180) / 360
            u1 = float(lng1+180) / 360
            if lng1 >= 180:
                lng1 -= 360
            p00 = LatLngToXYZ(float(lat0), float(lng0))
            p01 = LatLngToXYZ(float(lat0), float(lng1))
            p10 = LatLngToXYZ(float(lat1), float(lng0))
            p11 = LatLngToXYZ(float(lat1), float(lng1))
