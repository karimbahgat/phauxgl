...
from .vector import VectorW
from .triangle import NewTriangle
from .vertex import Barycentric, InterpolateVertexes
...
class clipPlane:
    ...
    def pointInFront(p, v):
        return v.Sub(p.P).Dot(p.N) > 0

    def intersectSegment(p, v0, v1):
        u = v1.Sub(v0)
        w = v0.Sub(p.P)
        d = p.N.Dot(u)
        n = -p.N.Dot(w)
...
clipPlanes = [
        clipPlane(VectorW(1, 0, 0, 1), VectorW(-1, 0, 0, 1)),
        clipPlane(VectorW(-1, 0, 0, 1), VectorW(1, 0, 0, 1)),
        clipPlane(VectorW(0, 1, 0, 1), VectorW(0, -1, 0, 1)),
        clipPlane(VectorW(0, -1, 0, 1), VectorW(0, 1, 0, 1)),
        clipPlane(VectorW(0, 0, 1, 1), VectorW(0, 0, -1, 1)),
        clipPlane(VectorW(0, 0, -1, 1), VectorW(0, 0, 1, 1)),
        ]

def sutherlandHodgman(points, planes):
    output = points
    for plane in planes:
        input = output
        output = []
        ...
        for e in input:
            if plane.pointInFront(e):
                if not plane.pointInFront(s):
                    x = plane.intersectSegment(s, e)
                    output.append(x)
                output.append(e)
            elif plane.pointInFront(s):
                x = plane.intersectSegment(s, e)
                output.append(x)
            s = e
    return output
...
def ClipTriangle(t):
    ...
    if not newPoints:
        return []
    result = []
    for i in range(2, len(newPoints)):
        ...
        result.append(NewTriangle(v1, v2, v3))
    return result
